...
with open (global_data_file_name,'rU')  as global_file:
    ...
    for k in range(0,len(globalData)):
        ...
        if  globalData[k][3]=='null':
            duration=datetime.datetime.strptime(globalData[k][0], " %Y-%m-%d %H:%M:%S.%f")-datetime.datetime.strptime(globalData[k-1][0], " %Y-%m-%d %H:%M:%S.%f")
            if duration<=datetime.timedelta(0,0,0,0,20):
                globalData[k][3]=globalData[k-1][3]

    for k in range(len(globalData)-1,0,-1):
        if  globalData[k][3]=='null':
            if (datetime.datetime.strptime(globalData[k+1][0], " %Y-%m-%d %H:%M:%S.%f")-datetime.datetime.strptime(globalData[k][0], " %Y-%m-%d %H:%M:%S.%f"))<=datetime.timedelta(0,0,0,0,20):
                globalData[k][3]=globalData[k+1][3]
        ...
        if condLine[0]==demographicData[i][1]:
            for j in range(0,12):
                ...
                for k in range(0,len(globalData)-1):
                    if demographicData[i][0]==globalData[k][3] and globalData[k][4]=='start' and (globalData[k+1][4]=='end' or globalData[k+1][4]=='FINISHEDTRIAL') and imageSetOrder[j/4]==int(globalData[k][1]) and j%4==int(globalData[k][2]):
                        duration=datetime.datetime.strptime(globalData[k+1][0], " %Y-%m-%d %H:%M:%S.%f")-datetime.datetime.strptime(globalData[k][0], " %Y-%m-%d %H:%M:%S.%f")
                        if duration<=datetime.timedelta(0,10):
                            duration=None
                        else:
                            score=globalData[k+1][5]
                outputData.append([demographicData[i][0],demographicData[i][1],demographicData[i][2],condLine[3+j/4],j/4+1,j+1,difficulty,
                                   imageSetOrder[j/4],j%4,imageID[j/4]+j%4,duration,score,demographicData[i][3],demographicData[i][6],demographicData[i][5],demographicData[i][4],demographicData[i][7]])
                outputData.append([demographicData[i+1][0],demographicData[i+1][1],demographicData[i+1][2],condLine[3+j/4],j/4+1,j+1,difficulty,
                                   imageSetOrder[j/4],j%4,imageID[j/4]+j%4,duration,score,demographicData[i+1][3],demographicData[i+1][6],demographicData[i+1][5],demographicData[i+1][4],demographicData[i+1][7]])
