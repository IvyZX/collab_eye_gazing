...
dateTimeFiles = ['041410', '041412', '041416', '041610', '041611', '042111', '042117', '043011', '043013', '043014',
                 '050510', '050514', '050711', '050712','050716','050717','051211','051213','051214']
gameStateDataFileName = '0512 data in seperate files/GameStateData'
gazeDataFileName = '0512 data in seperate files/ReleaseGazeData'
# reads the gaze file and store x and y coordinates into the gazeCoordinate variable.
# Does not read the received data.
def readGazeFile(dateTime):
    gazeFile = open(gazeDataFileName + dateTime + '.csv', 'r')
...
def readPuzzleFile(dateTime):
    puzzleFile = open(gameStateDataFileName + dateTime + '.csv', 'r')
    current_clicks = {}
    currentTime = None
        ...
        if line.__len__() >= 8:
            ...
            if (line[4].isdigit() and line[5].isdigit()):
                ...
                try:
                    ...
                    print 'Bad Data. Multiple TouchUps'
                    pass
    return
...
def isStaring(gazeData, puzzleData, clickData):
    ...
    for puzzle in puzzleData[1:]:
        if (puzzle[3] == clickData[2] and abs(gazeData[1] - puzzle[1]) <= accuracy and abs(
                    gazeData[2] - puzzle[2]) <= accuracy):
            # print(puzzle[3])
            staredPictures.append([gazeData[0], clickData[4], clickData[5], clickData[2], clickData[3], True, False])
            return True
        elif (puzzle[3] == clickData[3] and abs(gazeData[1] - puzzle[1]) <= accuracy and abs(
                    gazeData[2] - puzzle[2]) <= accuracy):
            # print(puzzle[3])
...
def findStaring():
    ...
    while (gazeIndex < len(gazeCoordinates) and puzzleIndex < len(puzzleCoordinates) and historyIndex < len(
        ...
        if (gazeIndex >= len(gazeCoordinates) or puzzleIndex >= len(puzzleCoordinates) or historyIndex >= len(
            ...
            return
                        ...
                        len(puzzleCoordinates[puzzleIndex + 1]) == 0 or len(puzzleCoordinates[puzzleIndex]) == 0):
            puzzleIndex += 1
    ...
    resultFile = open('accuracy.csv', 'a')
    ...
    for line in staredPictures[1:]:
        if (currentAName != str(line[1]) or currentBName != str(line[2])
            or (currentPuzzle != str(re.match('(\w+\d*)(?:_)', str(line[3])).group(1))
                and currentPuzzle != str(re.match('(\w+\d*)(?:_)', str(line[4])).group(1)))):
            if (counter != 0):
...
def readResult(resultFileName):
    accuracyFile = open('accuracy.csv', 'a')
    accuracyFile.write('A\'s ID,B\'s ID,Puzzle Name,Accuracy\n')
    counter = 0
    positives = 0
    currentPuzzle = ''
    currentAName = ''
    currentBName = ''
    resultFile = open('result.csv', 'r')
    for line in resultFile:
        ...
        try:
            if (currentAName != str(line[1]) or currentBName != str(line[2]) or currentPuzzle != str(
                ...
                if (counter != 0):
                    accuracyFile.write(currentAName + ',' + currentBName + ',' + currentPuzzle + ',' + str(
                        float(positives) / counter) + '\n')
                currentAName = str(line[1])
                currentBName = str(line[2])
                currentPuzzle = str(re.match('(\w+\d*)(?:_)', str(line[3])).group(1))
                ...
                counter = 0
                positives = 0
            if line[5] == str(True) or line[6] == str(True):
                positives += 1
            counter += 1
        except:
            pass
...
for dateTime in dateTimeFiles[13:]:
    gazeCoordinates = []
    puzzleCoordinates = []
    staredPictures = []
    clickHistory = []
    gazeIndex = 0;
    puzzleIndex = 0;
    readGazeFile(dateTime)
    readPuzzleFile(dateTime)
    findStaring()
# readResult('result.csv')
